...
COPY . .
RUN turbo prune chattergun --docker
...
RUN NODE_OPTIONS="--max-old-space-size=8192" yarn turbo run build --filter=chattergun --concurrency 2
...
COPY --from=installer /app/packages/chattergun/package.json .
COPY --from=installer /app/packages/chattergun/build /app/build
